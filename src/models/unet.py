...
class DoubleConv_w_options(nn.Module):
    ...
    def __init__(
        ...
        if normalization == "layer":
            layers.append(
                nn.LayerNorm(
                    normalized_shape=(mid_channels, img_shape[0], img_shape[1])
                )
            )
        ...
        if activation == "relu":
            ...
            layers.append(
                nn.Conv2d(
                    mid_channels, out_channels, kernel_size=3, padding=1, bias=bias
                )
            )
            if normalization == "batch":
            ...
            if normalization == "layer":
                layers.append(
                    nn.LayerNorm(
                        normalized_shape=(out_channels, img_shape[0], img_shape[1])
                    )
                )
            layers.append(nn.ReLU(inplace=True))
...
class Up_w_options(nn.Module):
    ...
    def __init__(
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)
            self.conv = DoubleConv_w_options(
                in_channels,
                out_channels,
                in_channels // 2,
                bias=bias,
...
class UNet2(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.description = f"Unet2_inFrames{in_frames}_outFrames{n_classes}_out_activation{output_activation}"
        self.n_channels = in_frames
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = DoubleConv(in_frames, filters, bias=bias)
        self.down1 = Down(filters, 2 * filters, bias=bias)
        self.down2 = Down(2 * filters, 4 * filters, bias=bias)
        self.down3 = Down(4 * filters, 8 * filters, bias=bias)
        factor = 2 if bilinear else 1
        ...
        self.up3 = Up(4 * filters, 2 * filters // factor, bilinear, bias=bias)
        self.up4 = Up(2 * filters, filters, bilinear, bias=bias)
        self.outc = OutConv(filters + in_frames, n_classes)

        self.dropout2D = nn.Dropout2d(p=dropout_p)
        if output_activation:
            if output_activation in [
                "sigmoid",
                "Sigmoid",
                "sigmoide",
                "Sigmoide",
...
class UNet_w_options(nn.Module):
    def __init__(
        ...
        self.inc = DoubleConv_w_options(
            n_channels,
            filters,
            activation=internal_activations,
            normalization=normalization,
            img_shape=self.img_shape,
        )

        self.down1 = Down_w_options(
            filters,
            2 * filters,
            bias=bias,
            activation=internal_activations,
        ...
        self.down2 = Down_w_options(
            2 * filters,
            4 * filters,
            bias=bias,
            activation=internal_activations,
            normalization=normalization,
            img_shape=(self.img_shape[0] // 4, self.img_shape[1] // 4),
        )

        self.down3 = Down_w_options(
            4 * filters,
            8 * filters,
            bias=bias,
            activation=internal_activations,
            normalization=normalization,
            img_shape=(self.img_shape[0] // 8, self.img_shape[1] // 8),
        )

        factor = 2 if bilinear else 1
        self.down4 = Down_w_options(
            8 * filters,
            16 * filters // factor,
            bias=bias,
            activation=internal_activations,
            normalization=normalization,
            img_shape=(self.img_shape[0] // 16, self.img_shape[1] // 16),
        )

        self.up1 = Up_w_options(
            16 * filters,
            8 * filters // factor,
            bilinear,
            bias=bias,
