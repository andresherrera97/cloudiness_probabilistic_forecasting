...
parser.add_argument(
    "--dataset", type=str, default="uru", help="Options: mvd, uru, region3"
)
...
parser.add_argument(
    "--architecture", type=str, default="UNET", help="UNET, R2, Att, Nested"
)
parser.add_argument(
    "--init_filters",
    type=int,
    default=16,
    help="number of filters in initial layer of U-Net",
)
parser.add_argument(
    "--output_last",
    type=bool,
    default=True,
    help="Dataloader only outputs objective time horizon",
)
parser.add_argument(
    "--time_horizon",
    type=str,
...
parser.add_argument(
    "--save_dict", type=bool, default=True, help="Save dict with training information"
)
parser.add_argument(
    "--train_loss", type=str, default="mae", help="Options: mae, mse, ssim"
)
parser.add_argument(
    "--loss_for_scheduler",
    type=str,
...
parser.add_argument(
    "--predict_diff", type=bool, default=False, help="Train U-Net to predict difference"
)
parser.add_argument(
    "--testing_loop",
    type=bool,
    default=False,
    help="Run fast pass through training loop",
)
parser.add_argument(
    "--writer", type=bool, default=True, help="Save results on a TensorBoard folder"
)
parser.add_argument(
    "--day_pct_filter",
    type=float,
    default=1,
    help="filter images with lower percentage of day pixels",
)
...
for lr in grid_search:
        ...
        if torch.cuda.device_count() == 1:
            for _ in range(len(checkpoint["model_state_dict"])):
                ...
                checkpoint["model_state_dict"][
                    key[7:] if key[:7] == "module." else key
                ] = value
    ...
    if args.writer:
        writer = SummaryWriter(
            log_dir=f"runs/paper/{args.time_horizon}/" + model_name, comment=comment
        )
    else:
        writer = None
    ...
    (TRAIN_LOSS, VAL_MAE_LOSS, VAL_MSE_LOSS, VAL_SSIM_LOSS,) = train_model_full(
        model=model,
