...
class NaNDebugger:
            ...
            if hasattr(module, 'weight') or hasattr(module, 'bias'):
                self.handles.append(
                    module.register_backward_hook(
                        lambda mod, grad_in, grad_out, name=name: self._gradient_hook(mod, grad_in, grad_out, name)
                    )
                )
    
    def _safe_clone(self, tensor):
        """Safely clone a tensor if it's a torch.Tensor"""
    ...
    def _activation_hook(self, module, inp, out, name):
        ...
        if self.step % self.log_freq == 0:
            with torch.no_grad():
                ...
                out_clone = self._safe_clone(out)
                
                if torch.isnan(out_clone).any():
                    self.nan_locations[name].append({
                        'step': self.step,
                        'type': 'activation',
                        'stats': self._compute_tensor_stats(out_clone)
                    })
            ...
            with torch.no_grad():
                ...
                for idx, grad in enumerate(grad_input):
                    if grad is not None:
                        ...
                        grad_clone = self._safe_clone(grad)
                        
                        if torch.isnan(grad_clone).any():
                            self.nan_locations[name].append({
                                'step': self.step,
                                'type': 'gradient_input',
                                'gradient_idx': idx,
                                'stats': self._compute_tensor_stats(grad_clone)
                            })
                            self.logger.warning(f"NaN detected in gradient input of layer {name}")
                        
                        # Check for exploding gradients
                        max_grad = torch.abs(grad_clone).max().item()
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if param.grad is not None:
                    param_stats[name] = {
                        'param_stats': self._compute_tensor_stats(param.data.clone()),
                        'grad_stats': self._compute_tensor_stats(param.grad.clone())
                    }
        return param_stats
    ...
    def step_callback(self, loss: Optional[torch.Tensor] = None):
        ...
        if self.step % self.log_freq == 0 and loss is not None:
            with torch.no_grad():
                # Check loss
                loss_clone = self._safe_clone(loss)
                if torch.isnan(loss_clone).any():
                ...
                for name, stats in param_stats.items():
                    if stats['param_stats'].get('all_nan', False) or \
                       stats['grad_stats'].get('all_nan', False):
                        self.logger.warning(f"NaN detected in parameters or gradients of {name}")
