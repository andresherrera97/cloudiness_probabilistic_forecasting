...
def analyze_psat(dcfg, return_creation=False):
    ...
    imgs_list = sorted(
        [img for img in os.listdir(dcfg["paths"]["psat"]) if img.endswith("rp.npy")]
    )
    ...
    if not return_creation:
        ...
        creation_dtime = dt.datetime.fromtimestamp(modification, tz=dt.timezone.utc)
...
def get_dtime(img_name):
    ...
    dtime = dt.datetime(
        year, 1, 1, hour=hs, minute=mins, second=sec, tzinfo=dt.timezone.utc
    ) + dt.timedelta(doy - 1)
    return dtime
...
def get_value(loc, winsize, img_path, lats=None, lons=None):
    ...
    img = np.load(img_path)
    value = np.nanmean(
        img[np.amax(i - hw, 0) : i + 1 + hw, np.amax(j - hw, 0) : j + 1 + hw]
    )
...
def next_step_starts(dtime, time_step):
    ...
    start = dt.datetime(
        dtime.year, dtime.month, dtime.day, dtime.hour, next_minute, tzinfo=dtime.tzinfo
...
def is_a_day_img(img_dtime, img_lats, img_lons, thresh=0.15):
    ...
    cosang00, mask00 = get_cosangs_nonvector(img_dtime, img_lats[0], img_lons[0])
    cosang01, mask01 = get_cosangs_nonvector(img_dtime, img_lats[0], img_lons[-1])
    cosang10, mask10 = get_cosangs_nonvector(img_dtime, img_lats[-1], img_lons[0])
    cosang11, mask11 = get_cosangs_nonvector(img_dtime, img_lats[-1], img_lons[-1])
...
def get_day_imgs_list(imgs_list):
    ...
    day_imgs_list = [
        img for img in imgs_list if is_a_day_img(get_dtime(img.name), lats, lons)
    ]
...
def solar_angles(lat, lon, delta_rad, eot_min, hour, minute, second, ret_mask=True):
...
def get_cosangs_nonvector(dtime, lat, lon):
    ...
    delta_rad, eot_min = daily_solar_angles(dtime.year, get_doy(dtime))
    cosangs, cos_mask = solar_angles(
        lat, lon, delta_rad, eot_min, dtime.hour, dtime.minute, dtime.second
    )
...
def write_to_daydf(df2, pred_dir_path, loc):
    ...
    for day, daygroup in df2.groupby(
        df2.index.day
    ):  # for each day the timezone conversion expanded the dataset into
        filename = Path(
            os.path.join(
                pred_dir_path,  # target filename for this day
                str(daygroup.index[0].date()) + ".csv",
            )
        )
        if not os.path.isfile(filename):  # create if nonexistent
            daydf = create_daydf(
                daygroup.index[0].date(),
...
def create_df2_datetimeindex(dtime_last, dtime_now, time_step, limit):
    ...
    """
    idx2 = pd.date_range(
        start=previous_step_starts(dtime_last, time_step),
        end=(dtime_now + dt.timedelta(hours=limit, minutes=time_step)),
...
def df1_to_df2(df1, dtime_now, time_step, limit):
    ...
    idx2 = create_df2_datetimeindex(dtime_now, dtime_now, time_step, limit)
    columns = df1.columns
...
def create_df1_from_dict(predictions, last_dtime, period, needed_lts):
    ...
    for column_name, dictionary in predictions.items():
        df1[column_name] = np.array([value for key, value in dictionary.items()])
    return df1.astype(float)


def create_daydf(
    datetimedate, period, columns=["predictions", "inf", "sup", "flags"], tzone=None
):
...
def initialize(name):
    ...
    file_handler = logging.FileHandler(f"admin_scripts/logs/{name}.log")
