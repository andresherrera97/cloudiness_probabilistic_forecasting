...
ap = argparse.ArgumentParser(
    description="Evaluate multiple models with multiple metrics"
)
...
ap.add_argument(
    "--data-path",
    default="/clusteruy/home03/DeepCloud/deepCloud/data/mvd/validation/",
...
ap.add_argument(
    "--save-name", default=None, help="Add name to results file. Defaults to None."
)
...
ap.add_argument(
    "--region", default=None, help="Defaults to region3. Region for cmv blur"
)
...
out_channels = list(
    range(start_horizon, start_horizon + horizon_step * predict_length, horizon_step)
)
...
for metric in metrics:
    ...
    for idx, a_model in enumerate(models):
        ...
        for out_channel in out_channels:
            ...
            error_array = evaluate.evaluate_model(
                a_model,
                val_loader,
        ...
        print(
            f"Error_mean_mean_{models_names[idx]}: {np.mean(error_mean_all_horizons)}"
        )
