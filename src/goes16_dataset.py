...
def generate_metadata(
    data_root, region="full_disk", out_folder="ABI_L2_CMIP_M6C02_G16", download_ref=True
):
    ...
    for k in ds.variables:
        if k in ["geospatial_lat_lon_extent", "goes_imager_projection"]:
            meta[k] = {
                subk: ds.variables[k].__dict__[subk]
                for subk in ds.variables[k].__dict__
            }
    ...
    lat_data_mask = (
        np.stack([lat.data, lat.mask])
        if hasattr(lat, "mask")
        else np.stack([lat, np.zeros_like(lat, dtype=bool)])
    )
    lon_data_mask = (
        np.stack([lon.data, lon.mask])
        if hasattr(lon, "mask")
        else np.stack([lon, np.zeros_like(lon, dtype=bool)])
    )
...
def download_goes16(
    ...
    if (
        (x - half < 0)
        or (y - half < 0)
        or (x + half >= ref_lat.shape[1])
        or (y + half >= ref_lat.shape[0])
    ):
    ...
    for day in dates:
        ...
        keys = [
            ...
            for o in bucket.objects.filter(
                Prefix=f"ABI-L2-CMIP{region}/{year}/{str(dofy).zfill(3)}/"
            )
        ...
        for k in tqdm(keys):
            ...
            # Check day/night
            cosangs, sunlit = get_cosangs(
                dt_obj,
                ref_lat[y - half : y + half, x - half : x + half],
            ...
            if save_as_npy:
                ...
                Image.fromarray(out_img, mode="I;16").save(
                    os.path.join(odir, fname + ".png")
                )
        ...
        # Save CSV
        pd.DataFrame(
            {"filenames": processed, "is_day": is_day, "inpaint_pct": inpaint_pct}
        ).to_csv(
            os.path.join(odir, f"data_{year}_{str(dofy).zfill(3)}.csv"), index=False
        )
