...
def evaluate_image(
    ...
    if C != len_gt:
        raise ValueError(
            "Predictions and Ground truth must have the same length. len(predictions)=",
            C,
            ", len(gt)=",
            len_gt,
        )
    ...
    for i in range(C):
        ...
        if evaluate_day_pixels:
            _, cosangs_thresh = utils.get_cosangs_mask(
                meta_path="data/meta", img_name=gt_ts[i]
            )
        ...
        if metric == "RMSE":
            ...
            nrmse = skimage.metrics.normalized_root_mse(
                gt_aux * (cosangs_map == 1), pred * (cosangs_map == 1)
            )
            error.append(nrmse)
        elif metric == "ReRMSE":
            eps = 0.0001
            re_rmse = np.sqrt(np.mean(((pred - gt_aux) * (cosangs_map == 1)) ** 2)) / (
                np.sqrt(np.mean((np.mean(gt_aux) - gt_aux) ** 2)) + eps
            )
            error.append(re_rmse)
        elif metric == "FS":
            rmse = np.sqrt(np.mean(((pred - gt_aux) * (cosangs_map == 1)) ** 2))
            rmse_persistence = np.sqrt(
                np.mean(((input_aux - gt_aux) * (cosangs_map == 1)) ** 2)
            )
...
def evaluate_pixel(predictions, gt, metric, pixel_max_value=255, pixel=(0, 0)):
    ...
    ):  # case the prediction input is an array instead of a list
        len_pred = 1
...
def evaluate_model(
    ...
    with tqdm(loader, desc=f"Status", unit="sequences") as loader_pbar:
        for idx, (inputs, targets) in enumerate(loader_pbar):
            ...
            if isinstance(model_instance, list):
                ...
                with torch.no_grad():
                    ...
                    for i in range(predict_horizon):
                        ...
                        if predict_diff:
                            prediction = torch.add(
                                prediction[:, 0], inputs[2]
                            ).unsqueeze(1)
                ...
                isinstance(model_instance, torch.nn.Module)
                and model_instance.n_classes == 1
            ):
                # recursive NN model
                predictions = []
                ...
                for i in range(predict_horizon):
                    predictions.append(
                        cv.GaussianBlur(
                            targets.cpu().detach().numpy()[i], kernel_size_list[i], 0
                        )
                    )
                predictions = np.array(predictions)
                dynamic_window = False
            ...
            gt = (
                targets[start_horizon].unsqueeze(0).cpu().detach().numpy()
                if start_horizon is not None
                else targets.cpu().detach().numpy()
            )
...
def make_val_grid(
    ...
    for idx, (in_frames, out_frames) in enumerate(val_loader):
        ...
        if idx % 100 == 0:
            ...
            grid.append(
                torch.cat(
                    (in_frames.detach(), out_frames.detach(), frames_pred.detach()),
                    dim=0,
                )
            )
            if flag == sequences:
