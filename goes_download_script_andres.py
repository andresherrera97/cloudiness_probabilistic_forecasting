...
class Downloader:
    ...
    @timeit
    def _read_crop(self, bucket: str, f: str, x: int, y: int, size: int, verbose: bool):
        """
    ...
    @timeit
    def crop_processing(
        self, CMI_DQF_crop: np.ndarray, cosangs: np.ndarray, calculate_pr: bool = True
    ) -> np.ndarray:
        """
        ...
        # Step 2: normalize
        if calculate_pr:
            crop = normalize(CMI_DQF_crop[0], cosangs, 0.15)
            crop[np.isnan(crop)] = 0
            crop = np.clip(crop, 0, 1.0).astype(np.float16)
        else:
            crop = CMI_DQF_crop[0].astype(np.float16)

        # Step 3: remove nans and clip
        logger.info(f"Processed Crop range: {crop.min()} - {crop.max()}")
        return crop, pct_inpaint
    ...
    def _download_and_process_file(
        ...
        cmi_dqf_crop = self._read_crop(bucket, f, x, y, size, verbose)
        ...
        pr, pct_inp = self.crop_processing(cmi_dqf_crop, cosangs, calculate_pr)
    ...
    def download_files(
        ...
        if satellite.lower() == "g16":
            bucket = "noaa-goes16"
        elif satellite.lower() == "g17":
            bucket = "noaa-goes17"
        elif satellite.lower() == "g18":
            bucket = "noaa-goes18"
        else:
            raise ValueError("Invalid satellite. Must be G16, G17, or G18.")
...
class Metadata:
    ...
    def create_metadata(
        ...
        if satellite.lower() == "g16":
            ...
            bucket = "noaa-goes17"
            PATH_TO_LOCAL_CONUS = "datasets/CONUS_nc_files/OR_ABI-L2-CMIPC-M6C02_G17_s20220152021177_e20220152023550_c20220152024051.nc"
            PATH_TO_LOCAL_FULL_DISK = "datasets/full_disk_nc_files/OR_ABI-L2-CMIPF-M6C02_G17_s20200371650321_e20200371659388_c20200371659448.nc"
            S3_PATH_TO_CONUS = "ABI-L2-CMIPC/2022/015/20/OR_ABI-L2-CMIPC-M6C02_G17_s20220152021177_e20220152023550_c20220152024051.nc"
            S3_PATH_TO_FULL_DISK = "ABI-L2-CMIPF/2020/037/16/OR_ABI-L2-CMIPF-M6C02_G17_s20200371650321_e20200371659388_c20200371659448.nc"
        elif satellite.lower() == "g18":
            bucket = "noaa-goes18"
            PATH_TO_LOCAL_CONUS = "datasets/CONUS_nc_files/OR_ABI-L2-CMIPC-M6C02_G18_s20240460841185_e20240460843558_c20240460844051.nc"
            PATH_TO_LOCAL_FULL_DISK = "datasets/full_disk_nc_files/OR_ABI-L2-CMIPF-M6C02_G18_s20230071940215_e20230071949523_c20230071949591.nc"
            S3_PATH_TO_CONUS = "ABI-L2-CMIPC/2024/046/08/OR_ABI-L2-CMIPC-M6C02_G18_s20240460841185_e20240460843558_c20240460844051.nc"
            S3_PATH_TO_FULL_DISK = "ABI-L2-CMIPF/2023/007/19/OR_ABI-L2-CMIPF-M6C02_G18_s20230071940215_e20230071949523_c20230071949591.nc"
        else:
            raise ValueError("Invalid satellite. Must be G16, G17, or G18.")
...
class Listing:
    ...
    def get_S3_files_in_range(
        ...
        """
        assert region in ["F", "C"], "Invalid region. Must be 'F' or 'C'."
        ...
        with ThreadPoolExecutor(
            ...
            future_to_date = {
                executor.submit(
                    self._get_day_filenames, bucket, region, s3_client, dt.timetuple().tm_yday, dt.year
                ): dt
    ...
    def _get_day_filenames(self, bucket: str, region: str, s3_client, doy, year):
        """
        Retrieves S3 file list for a specific day using list_objects_v2.
        """
        prefix = f"{SENSOR}-{PROCESSING_LEVEL}-{PRODUCT}{region}" + f"/{year}/{doy:03}/"
        files = []
